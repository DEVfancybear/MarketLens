...
func TestNormalizePositionLifecycle(t *testing.T) {
	...
	if _, err := normalizePosition(valid); err != nil {
		t.Fatalf("valid position: %v", err)
	}
	valid.Remaining = 1
	if _, err := normalizePosition(valid); err == nil {
		t.Fatal("closed position with remainder must fail")
	}
	valid.Remaining, valid.Entry = 0, math.Inf(1)
	if _, err := normalizePosition(valid); err == nil {
		t.Fatal("infinite entry must fail")
	}
}
...
func TestAnalyticsMatchesClientDefinitions(t *testing.T) {
	...
	if r.Summary.TotalTrades != 3 || r.Summary.Wins != 1 || r.Summary.Losses != 1 || r.Summary.Breakeven != 1 {
		t.Fatalf("counts: %+v", r.Summary)
	}
	if math.Abs(r.Summary.WinRate-100.0/3.0) > 1e-9 || r.Summary.ProfitFactor != 2 || r.Summary.NetPnL != 100 {
		t.Fatalf("summary: %+v", r.Summary)
	}
	if r.Summary.MaxDrawdown != -100 || len(r.Equity) != 3 || r.Equity[2].Equity != 10_100 {
		t.Fatalf("curve: %+v summary=%+v", r.Equity, r.Summary)
	}
	if r.Distribution[5].Count != 1 || r.Distribution[1].Count != 1 || r.Distribution[3].Count != 1 {
