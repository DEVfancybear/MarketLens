...
type IntegrationRecord struct {
	...
	TelegramBotToken, DiscordWebhook []byte
...
func (r *IntegrationRepo) Get(ctx context.Context, userID string) (IntegrationRecord, error) {
	...
	return scanIntegration(r.pool.QueryRow(ctx, `SELECT telegram_chat_id, telegram_bot_cipher, telegram_enabled, discord_webhook_cipher, discord_enabled FROM user_integrations WHERE user_id=$1`, uid))
}
...
func (r *IntegrationRepo) Put(ctx context.Context, userID string, v IntegrationRecord) (IntegrationRecord, error) {
	...
	return scanIntegration(r.pool.QueryRow(ctx, `INSERT INTO user_integrations (user_id,telegram_chat_id,telegram_bot_cipher,telegram_enabled,discord_webhook_cipher,discord_enabled) VALUES ($1,$2,$3,$4,$5,$6) ON CONFLICT(user_id) DO UPDATE SET telegram_chat_id=EXCLUDED.telegram_chat_id,telegram_bot_cipher=EXCLUDED.telegram_bot_cipher,telegram_enabled=EXCLUDED.telegram_enabled,discord_webhook_cipher=EXCLUDED.discord_webhook_cipher,discord_enabled=EXCLUDED.discord_enabled,updated_at=now() RETURNING telegram_chat_id,telegram_bot_cipher,telegram_enabled,discord_webhook_cipher,discord_enabled`, uid, v.TelegramChatID, v.TelegramBotToken, v.TelegramEnabled, v.DiscordWebhook, v.DiscordEnabled))
}
...
func scanIntegration(row integrationScanner) (v IntegrationRecord, err error) {
	err = row.Scan(&v.TelegramChatID, &v.TelegramBotToken, &v.TelegramEnabled, &v.DiscordWebhook, &v.DiscordEnabled)
	return
}
