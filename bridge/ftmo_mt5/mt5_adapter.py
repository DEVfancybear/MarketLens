...
class Mt5Adapter:
    ...
    def connect(self) -> None:
        ...
        if not self.config.allow_live:
            raise RuntimeError("FTMO_BRIDGE_ALLOW_LIVE must be true before live MT5 adapter connects")
        try:
            import MetaTrader5 as mt5  # type: ignore[import-not-found]
        except ImportError as exc:
            raise Mt5ImportError("Install bridge/ftmo_mt5/requirements.txt on Windows with MT5 installed") from exc
        ...
        if self.config.login:
            ...
            if not mt5.login(login, password=self.config.password, server=self.config.server):
    ...
    def readiness_checks(self, audit_writable: bool) -> list[dict[str, Any]]:
        checks = [
            {
                ...
                "detail": "FTMO_MT5_ENABLED=true" if self.config.enabled else "Set FTMO_MT5_ENABLED=true",
            },
            {
                "name": "audit_log",
                "ok": audit_writable,
                "detail": "Audit log writable" if audit_writable else "Audit log is not writable",
            },
            {
                "name": "dry_run_default",
                "ok": self.config.dry_run or self.config.allow_live,
                "detail": "Dry-run mode is active" if self.config.dry_run else "Live mode explicitly enabled",
            },
        ]
        if self.config.dry_run:
            checks.extend(
                [
                    {"name": "mt5_import", "ok": True, "detail": "Not required for dry-run"},
                    {"name": "mt5_terminal", "ok": True, "detail": "Not required for dry-run"},
                    {"name": "mt5_login", "ok": True, "detail": "Not required for dry-run"},
                ],
            )
            return checks
        ...
        checks.extend(
            [
                ...
                },
                {
                    ...
                    "ok": self.initialized,
                    "detail": "MetaTrader5 session initialized" if self.initialized else "MetaTrader5 session not initialized",
                },
            ],
        )
        ...
        if account is not None:
            checks.append(
                {
                    ...
                    "ok": not self.config.login or str(getattr(account, "login", "")) == self.config.login,
            ...
            checks.append({"name": "account_info", "ok": False, "detail": "MT5 account_info unavailable"})
        return checks
    ...
    def account_snapshot(self) -> dict[str, Any]:
        ...
        return {
            ...
            "mode": "demo" if not bool(getattr(account, "trade_mode", 0)) else "unknown",
            ...
            result.append(
                {
                    ...
                    "side": "long" if int(getattr(row, "type", 0) or 0) == self.mt5.POSITION_TYPE_BUY else "short",
            ...
            result.append(
                {
                    ...
                    "symbol": chart_symbol,
                    "brokerSymbol": getattr(row, "symbol", ""),
                    "side": "buy" if int(getattr(row, "type", 0) or 0) in {self.mt5.ORDER_TYPE_BUY_LIMIT, self.mt5.ORDER_TYPE_BUY_STOP} else "sell",
                    "type": "limit" if int(getattr(row, "type", 0) or 0) in {self.mt5.ORDER_TYPE_BUY_LIMIT, self.mt5.ORDER_TYPE_SELL_LIMIT} else "stop",
    ...
    def place_order(self, order: dict[str, Any], volume: float) -> tuple[bool, dict[str, Any]]:
        if self.mt5 is None or not self.initialized:
            return False, {"code": "MT5_NOT_INITIALIZED", "message": "MT5 session is not initialized"}
        meta = self.symbol_meta(order["chartSymbol"])
        if meta is None:
            return False, {"code": "UNKNOWN_SYMBOL", "message": f"No MT5 symbol for {order['chartSymbol']}"}
        try:
        ...
        if check is None:
            return False, {"code": "ORDER_CHECK_FAILED", "message": str(self.mt5.last_error()), "request": request}
        if not self._success_retcode(int(getattr(check, "retcode", 0) or 0), allow_zero=True):
            return False, {
        ...
        if result is None:
            return False, {"code": "ORDER_SEND_FAILED", "message": str(self.mt5.last_error()), "request": request}
    ...
    def close_position(self, ticket: str, volume: float | None = None, deviation_points: int | None = None) -> tuple[bool, dict[str, Any]]:
        if self.mt5 is None or not self.initialized:
            return False, {"code": "MT5_NOT_INITIALIZED", "message": "MT5 session is not initialized"}
        rows = self.mt5.positions_get(ticket=int(ticket)) or []
        if not rows:
            return False, {"code": "POSITION_NOT_FOUND", "message": f"Position {ticket} not found"}
        position = rows[0]
        symbol = getattr(position, "symbol", "")
        tick = self.mt5.symbol_info_tick(symbol)
        side = int(getattr(position, "type", 0) or 0)
        close_type = self.mt5.ORDER_TYPE_SELL if side == self.mt5.POSITION_TYPE_BUY else self.mt5.ORDER_TYPE_BUY
        price = float(getattr(tick, "bid" if close_type == self.mt5.ORDER_TYPE_SELL else "ask", 0) or 0)
        request = {
        ...
        retcode = int(getattr(result, "retcode", 0) or 0)
        if not self._success_retcode(retcode):
            return False, {"code": "ORDER_SEND_REJECTED", "message": getattr(result, "comment", ""), "retcode": retcode, "request": request, "result": _as_dict(result)}
        return True, {"ticket": ticket, "price": float(getattr(result, "price", price) or price), "result": _as_dict(result)}

    def modify_position(self, ticket: str, sl: float | None, tp: float | None) -> tuple[bool, dict[str, Any]]:
        if self.mt5 is None or not self.initialized:
            return False, {"code": "MT5_NOT_INITIALIZED", "message": "MT5 session is not initialized"}
        rows = self.mt5.positions_get(ticket=int(ticket)) or []
        if not rows:
    ...
    def cancel_order(self, ticket: str) -> tuple[bool, dict[str, Any]]:
        if self.mt5 is None or not self.initialized:
            return False, {"code": "MT5_NOT_INITIALIZED", "message": "MT5 session is not initialized"}
        request = {"action": self.mt5.TRADE_ACTION_REMOVE, "order": int(ticket), "magic": self.config.magic}
        result = self.mt5.order_send(request)
        if result is None:
            return False, {"code": "ORDER_SEND_FAILED", "message": str(self.mt5.last_error()), "request": request}
        retcode = int(getattr(result, "retcode", 0) or 0)
    ...
    def _build_order_request(self, order: dict[str, Any], meta: SymbolMeta, volume: float) -> dict[str, Any]:
        assert self.mt5 is not None
        ...
        is_buy = side == "buy"
        price = float(order.get("price") or (getattr(tick, "ask", 0) if is_buy else getattr(tick, "bid", 0)))
        if order_type == "market":
            mt5_type = self.mt5.ORDER_TYPE_BUY if is_buy else self.mt5.ORDER_TYPE_SELL
            action = self.mt5.TRADE_ACTION_DEAL
        elif order_type == "limit":
            mt5_type = self.mt5.ORDER_TYPE_BUY_LIMIT if is_buy else self.mt5.ORDER_TYPE_SELL_LIMIT
        ...
        return {
            ...
            "deviation": int(order.get("deviationPoints") or self.config.deviation_points),
            "magic": self.config.magic,
            "comment": f"{self.config.comment_prefix}:{order.get('clientOrderId', '')}"[:31],
            "type_time": self.mt5.ORDER_TIME_GTC,
            "type_filling": self.mt5.ORDER_FILLING_IOC,
        }
