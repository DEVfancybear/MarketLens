...
test("Wave D adapters render, bound, anchor, move, hit-test, and serialize",()=>{for(const tool of TOOLS){const adapter=getTool(tool);assert.ok(adapter,tool);const drawing=fixture(tool);const definition=getDrawingToolManifestEntry(tool);assert.doesNotThrow(()=>adapter.render(context(),drawing,projector,true),tool);const b=adapter.boundingBox(drawing,projector.toX,projector.toY);assert.ok(b,`${tool} bounds`);assert.ok([b.x,b.y,b.w,b.h].every(Number.isFinite),tool);const anchors=adapter.getAnchors(drawing,projector.toX,projector.toY);if(definition.handleProfile==="raw-points"){assert.equal(anchors.length,drawing.points.length);}else{assert.equal(anchors.length,4,`${tool} virtual handles`);}assert.equal(adapter.move(drawing.points,{time:2,price:2},{time:1,price:1}).length,drawing.points.length);assert.doesNotThrow(()=>JSON.parse(JSON.stringify(drawing)));}}
);
