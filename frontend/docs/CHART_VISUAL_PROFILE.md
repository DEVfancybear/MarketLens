...
Separate indicator panes must preserve the main chart's candle timestamps even
when an indicator returns sparse data. `IndicatorPane` uses a transparent
time-anchor series generated by `indicatorPaneTimeScale.ts`; keep it in place
for RSI/MACD/custom scripts that omit `na` points, or logical-range sync will
drift from the main chart.
