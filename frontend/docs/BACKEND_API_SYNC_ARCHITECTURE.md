...
Logout is the inverse boundary. When an authenticated session resolves to `anonymous`,
`useWorkspaceBootstrap()` resets user-scoped atoms to defaults and clears their browser cache keys.
An initial anonymous/offline load may rehydrate the local cache for drawing preferences, but it never
merges that cache into an authenticated bootstrap. This prevents a signed-out screen from showing
the previous user's watchlists or workspace settings. The reset intentionally leaves
