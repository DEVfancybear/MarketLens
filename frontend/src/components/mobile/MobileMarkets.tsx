            ...
            {results.map((item) => {
              ...
              return <article key={item.id}>
                <button type="button" className="mobile-market-search-main" onClick={() => openSymbol(item.id)}>
                  <span className="mobile-symbol-avatar">{item.id.slice(0, 2)}</span><span><strong>{item.id}</strong><small>{item.name} · {item.exchange}</small></span>{item.id === active && <Check size={18} />}
                </button>
          ...
          <div className="mobile-watchlist-manager">
            <section>
              ...
              <div className="mobile-watchlist-actions">
                <button type="button" onClick={() => { const name = window.prompt("Watchlist name", "New watchlist")?.trim(); if (name) createList(name); }}><Plus size={17} />New</button>
                <button type="button" onClick={() => { const name = window.prompt("Rename watchlist", list.name)?.trim(); if (name) renameList(name); }}><Pencil size={17} />Rename</button>
                <button type="button" disabled={!list.symbols.length} onClick={() => { if (window.confirm(`Clear “${list.name}”?`)) clearList(); }}><Trash2 size={17} />Clear</button>
                <button type="button" className="is-danger" disabled={lists.length <= 1} onClick={() => { if (window.confirm(`Delete “${list.name}”?`)) removeList(activeListId); }}><Trash2 size={17} />Delete</button>
              </div>
            </section>
            <section>
            ...
            <section>
              ...
              <button type="button" className="mobile-add-section" onClick={() => { const title = window.prompt("Section name", "New section")?.trim(); if (title) addSection(title); }}><Plus size={17} />Add section</button>
              <div className="mobile-watchlist-sections">
                {[...sections].sort((a, b) => a.index - b.index).map((section, index, ordered) => <article key={section.id}>
                  ...
                  <div>
                    ...
                    <button type="button" aria-label={`Rename ${section.title}`} onClick={() => { const title = window.prompt("Rename section", section.title)?.trim(); if (title) renameSection(section.id, title); }}><Pencil size={17} /></button>
                    <button type="button" aria-label={`Delete ${section.title}`} className="is-danger" onClick={() => { if (window.confirm(`Delete section “${section.title}”?`)) removeSection(section.id); }}><Trash2 size={17} /></button>
                  </div>
                </article>)}
