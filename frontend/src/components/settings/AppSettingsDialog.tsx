...
export function AppSettingsDialog() {
  ...
  const applySaved = (v: Awaited<ReturnType<typeof saveIntegrationSettings>>) => {
    setConfigured({mt5:v.mt5.passwordConfigured,telegram:v.telegram.botTokenConfigured,discord:v.discord.webhookConfigured});
    setDraft((d)=>({...d,mt5:{...d.mt5,password:"",clearPassword:false},telegram:{...d.telegram,botToken:"",clearBotToken:false},discord:{...d.discord,webhookUrl:"",clearWebhook:false}}));
  };
  const save = async () => { setBusy(true); setMessage(""); try { const v=await saveIntegrationSettings(draft); applySaved(v); setMessage("Settings saved."); } catch (error) { setMessage(errorMessage(error, "Save failed. Check the fields and backend session.")); } finally { setBusy(false); } };
  const test = async (channel:"telegram"|"discord") => { setBusy(true); setMessage(""); try { const v=await saveIntegrationSettings(draft); applySaved(v); await testIntegration(channel); setMessage(`${channel} settings saved and test sent.`); } catch (error) { setMessage(errorMessage(error, `${channel} test failed.`)); } finally { setBusy(false); } };
    ...
    <div className="max-h-[90vh] w-[min(720px,calc(100vw-32px))] overflow-y-auto rounded-lg border border-terminal-border bg-terminal-panel shadow-2xl" onMouseDown={(e)=>e.stopPropagation()}>
      ...
      {!backendSession ? <div className="p-8 text-sm text-ink-muted">Sign in and establish a backend session to manage private integrations.</div> : <div className="space-y-6 p-5">
        ...
        <Section title="Telegram" note="Bot token + target chat ID for alert delivery.">
          <Secret label="Bot token" configured={configured.telegram} value={draft.telegram.botToken} onChange={(v)=>setDraft({...draft,telegram:{...draft.telegram,botToken:v,clearBotToken:false}})} onClear={()=>setDraft({...draft,telegram:{...draft.telegram,botToken:"",clearBotToken:true}})} cls={field} emptyPlaceholder="Example: 123456789:AA..."/><Input label="Chat ID" value={draft.telegram.chatId} onChange={(v)=>setDraft({...draft,telegram:{...draft.telegram,chatId:v}})} cls={field} placeholder="Numeric ID, for example 123456789"/><Toggle label="Enable Telegram alerts" checked={draft.telegram.enabled} onChange={(v)=>setDraft({...draft,telegram:{...draft.telegram,enabled:v}})}/><button disabled={busy||!(configured.telegram||Boolean(draft.telegram.botToken.trim()))||!draft.telegram.chatId.trim()} onClick={()=>void test("telegram")} className="rounded border border-terminal-border px-3 py-1.5 text-xs text-ink disabled:opacity-40">Save & send test</button>
        </Section>
        <Section title="Discord" note="Incoming webhook URL for alert delivery.">
          <Secret label="Webhook URL" configured={configured.discord} value={draft.discord.webhookUrl} onChange={(v)=>setDraft({...draft,discord:{...draft.discord,webhookUrl:v,clearWebhook:false}})} onClear={()=>setDraft({...draft,discord:{...draft.discord,webhookUrl:"",clearWebhook:true}})} cls={field}/><Toggle label="Enable Discord alerts" checked={draft.discord.enabled} onChange={(v)=>setDraft({...draft,discord:{...draft.discord,enabled:v}})}/><button disabled={busy||!(configured.discord||Boolean(draft.discord.webhookUrl.trim()))} onClick={()=>void test("discord")} className="rounded border border-terminal-border px-3 py-1.5 text-xs text-ink disabled:opacity-40">Save & send test</button>
        </Section>
      </div>}
...
function Input({label,value,onChange,cls,placeholder}:{label:string;value:string;onChange:(v:string)=>void;cls:string;placeholder?:string}) { return <label className="block text-xs text-ink-muted"><span className="mb-1 block">{label}</span><input className={cls} value={value} placeholder={placeholder} onChange={(e)=>onChange(e.target.value)}/></label>; }
function Secret({label,configured,value,onChange,onClear,cls,emptyPlaceholder="Not configured"}:{label:string;configured:boolean;value:string;onChange:(v:string)=>void;onClear:()=>void;cls:string;emptyPlaceholder?:string}) { return <div><label className="block text-xs text-ink-muted"><span className="mb-1 block">{label}</span><input type="password" autoComplete="new-password" className={cls} value={value} placeholder={configured?"Configured — enter to replace":emptyPlaceholder} onChange={(e)=>onChange(e.target.value)}/></label>{configured&&<button type="button" onClick={onClear} className="mt-1 text-[11px] text-bear hover:underline">Clear saved secret</button>}</div>; }
