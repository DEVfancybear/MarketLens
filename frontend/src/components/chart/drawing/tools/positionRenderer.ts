...
} from "./positionProjectedGeometry";
import { layoutPositionLabels } from "./positionLabelLayout";
import {
  detectPositionHit as detectPositionHitFromService,
  positionHitDataCoversEntry as positionHitDataCoversEntryFromService,
  resolvePositionHit as resolvePositionHitFromService,
} from "./positionHitResolution";
...
export function positionHitDataCoversEntry(
  ...
  return positionHitDataCoversEntryFromService(d, candles);
}
...
export function resolvePositionHit(
  ...
  return resolvePositionHitFromService(d, candles);
}
...
function render(
  ...
  const geo = projectPositionGeometry(d, proj);
  if (!geo) return;
  ...
  if (d.showLabels !== false) {
    ...
    const labelPad = 2;
    const placements = layoutPositionLabels(
      [
        { id: "target", y: clampToPricePane(yT), height: chipH },
        { id: "stop", y: clampToPricePane(yS), height: chipH },
        { id: "entry", y: clampToPricePane(yE), height: chipH },
      ] as const,
      labelPad,
      clipBottom - labelPad,
    );
    const labelTop = (id: "target" | "stop" | "entry") =>
      placements.find((placement) => placement.id === id)?.top ?? labelPad;
  ...
  if (selected) {
    ...
    for (const anchor of positionAnchorsFromGeometry({
      ...geo,
      xR: origXR,
    })) {
...
function hitTest(
  ...
  return hitTestPositionGeometry(d, px, py, toX, toY);
}
...
function getAnchors(
  ...
  const geo = projectPositionGeometry(d, { toX, toY });
  if (!geo) return [];
  return positionAnchorsFromGeometry(geo);
}
...
function boundingBox(d: Drawing, toX: HitTestProjector, toY: HitTestProjector) {
  return positionGeometryBounds(d, toX, toY);
}
