...
const DEFAULT_PANELS: PanelSizes = { right: 280, bottom: 240, left: 48 };
...
export const useUIStore = create<UIState>((set, get) => ({
  ...
  hydrate: () => {
    const persisted = localStore.get('ui', { theme: get().theme, panels: get().panels });
  ...
  log: (level, msg) =>
    set((s) => ({
      logs: [{ id: ++logId, time: Date.now() / 1000, level, msg }, ...s.logs].slice(0, 200),
    })),
}));
