...
const PRESET_COLORS = [
  '#2962ff', '#26a69a', '#ef5350', '#ff9800', '#ab47bc',
  '#00bcd4', '#8bc34a', '#ffeb3b', '#ffffff', '#78909c',
];
...
export function IndicatorSettingsDialog() {
  const editingId = useChartStore((s) => s.editingIndicatorId);
  ...
  const save = () => {
    const patch: Partial<IndicatorConfig> = {
      ...
      separatePane: type === 'RSI' || type === 'MACD' ? separatePane : undefined,
      visible,
    };
    updateIndicator(editingId, patch);
    close();
  };
    ...
    >
      <div className="w-[340px] rounded-lg border border-terminal-border bg-terminal-panel-2 shadow-2xl shadow-black/60">
        <div className="flex items-center justify-between border-b border-terminal-border px-4 py-2.5">
          ...
          <button onClick={close} className="rounded p-0.5 text-ink-muted hover:text-ink hover:bg-terminal-hover">
            <X size={14} />
          </button>
        </div>
        ...
        <div className="space-y-3 px-4 py-3">
          {/* Type selector */}
          <div>
            <div className="mb-1 text-2xs font-semibold uppercase tracking-wide text-ink-faint">Type</div>
          ...
          {/* Length */}
          {(type === 'SMA' || type === 'EMA' || type === 'RSI' || type === 'MACD' || type === 'ADR') && (
            <SliderInput label="Length" value={length} min={1} max={500} onChange={setLength} />
          )}

          {/* MACD slow */}
          {type === 'MACD' && (
            <SliderInput label="Slow" value={length3} min={1} max={500} onChange={setLength3} />
          )}

          {/* MACD signal */}
          {type === 'MACD' && (
            <SliderInput label="Signal" value={length2} min={1} max={200} onChange={setLength2} />
          )}

          {/* Color 1 */}
          <div>
            <div className="mb-1 text-2xs font-semibold uppercase tracking-wide text-ink-faint">Color</div>
          ...
          {showColor2 && (
            <div>
              ...
              <div className="flex items-center gap-1 flex-wrap">
                {PRESET_COLORS.map((c) => (
                  <ColorChip key={c} color={c} selected={color2 === c} onClick={() => setColor2(c)} />
                ))}
                <input
                  type="color"
                  value={color2}
            ...
            <div className="flex items-center gap-3">
              ...
              <button
                ...
                className={cn(
                  ...
                  !separatePane ? 'border-brand/40 bg-brand/15 text-brand' : 'border-terminal-border text-ink-muted hover:bg-terminal-hover',
                )}
              >
                Overlay
              </button>
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className={cn(
                'rounded border px-2 py-0.5 text-2xs font-medium transition-colors',
                visible ? 'border-brand/40 bg-brand/15 text-brand' : 'border-terminal-border text-ink-muted hover:bg-terminal-hover',
              )}
            >
              Show
            </button>
