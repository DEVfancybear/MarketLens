...
function ToastCard({ toast }: { toast: Toast }) {
  ...
  return (
    ...
    >
      <span className="mt-0.5 shrink-0" style={{ color: meta.accent }}>{meta.icon}</span>
      <div className="min-w-0 flex-1">
        <div className="text-xs font-semibold text-ink">{toast.title}</div>
        {toast.message && <div className="mt-0.5 break-words text-2xs text-ink-muted">{toast.message}</div>}
      </div>
      <button
