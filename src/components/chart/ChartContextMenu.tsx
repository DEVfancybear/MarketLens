...
import { Bell, TrendingDown, TrendingUp, Plus, Minus } from 'lucide-react';
...
type MenuItem =
  | { divider: true }
  | { divider?: false; icon: React.ReactNode; label: string; danger?: boolean; onClick: () => void };
...
export function ChartContextMenu({
  ...
  const items: MenuItem[] = [
    ...
    { divider: true },
    {
      ...
      onClick: act(() => {
        place({ symbol, side: 'short', type: 'limit', price: state.price, quantity: 1 });
        setBottomTab('trade');
      }),
    },
    {
      ...
      onClick: act(() => {
        place({ symbol, side: 'long', type: 'stop', price: state.price, quantity: 1 });
        setBottomTab('trade');
      }),
    },
    {
    ...
    >
      ...
      {items.map((it, i) =>
        'divider' in it && it.divider ? (
          ...
          >
            <span className="shrink-0">{(it as Exclude<MenuItem, { divider: true }>).icon}</span>
            <span className="flex-1">{(it as Exclude<MenuItem, { divider: true }>).label}</span>
          </button>
        ),
      )}
    </div>,
