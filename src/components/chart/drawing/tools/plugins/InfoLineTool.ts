/**
 * InfoLineTool — measurement line with price change and percentage label.
 */
...
const plugin: DrawingToolPlugin = {
  ...
  boundingBox(d: Drawing, toX: HitTestProjector, toY: HitTestProjector) {
    ...
    return { x: Math.min(x1, x2) - 80, y: Math.min(y1, y2) - 18, w: Math.abs(x2 - x1) + 160, h: Math.abs(y2 - y1) + 36 };
  },
};
