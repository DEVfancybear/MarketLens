...
import { Camera, Download, FileSpreadsheet, Trash2, ChevronDown, ChevronRight } from 'lucide-react';
...
export function JournalPanel() {
  ...
  const attach = useJournalStore((s) => s.attachScreenshot);
  ...
  const capture = async (entryId: string, phase: ScreenshotPhase) => {
    ...
    if (!blob) { log('warn', 'Chart not ready for screenshot'); return; }
    const thumb = URL.createObjectURL(blob);
    await attach(entryId, { id: uid('shot'), phase, thumb, createdAt: Date.now() / 1000 }, blob);
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <div className="flex h-9 shrink-0 items-center gap-3 border-b border-terminal-border px-3 text-2xs">
        ...
        <span className="tabular font-semibold" style={{ color: totalPnl >= 0 ? 'var(--bull)' : 'var(--bear)' }}>
          {fmtMoney(totalPnl)}
        </span>
        <div className="ml-auto flex items-center gap-1">
          <button onClick={() => exportCSV(entries)} disabled={!entries.length}
          ...
          <button onClick={() => exportExcel(entries)} disabled={!entries.length}
            className="flex items-center gap-1 rounded px-2 py-1 text-ink-muted hover:bg-terminal-hover hover:text-ink disabled:opacity-40">
            ...
            <div key={e.id} className="border-b border-terminal-border">
              ...
              >
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span style={{ color: e.side === 'long' ? 'var(--bull)' : 'var(--bear)' }}>{e.side.toUpperCase()}</span>
                  <span className="text-ink-faint">{fmtDateTime(e.entryTime)}</span>
                </div>
                <span className="tabular text-ink-muted">{fmtR(e.rr)}</span>
                <span className="tabular font-semibold" style={{ color: e.pnl >= 0 ? 'var(--bull)' : 'var(--bear)' }}>
                  {fmtMoney(e.pnl)}
                </span>
                <button onClick={(ev) => { ev.stopPropagation(); remove(e.id); }} title="Delete">
                  <Trash2 size={12} className="text-ink-faint hover:text-bear" />
                </button>
              </div>
              ...
              {isOpen && (
                <div className="space-y-2 bg-terminal-panel-2/40 px-9 py-2">
                  <div className="grid grid-cols-4 gap-2 text-2xs">
                    <Detail label="Entry" value={fmtPrice(e.entryPrice, prec)} />
                  ...
                  <div className="flex items-center gap-2">
                    {PHASES.map((p) => (
                      <button key={p.phase} onClick={() => capture(e.id, p.phase)}
                  ...
                  {e.screenshots && e.screenshots.length > 0 && (
                    <div className="flex flex-wrap gap-2">
                      {e.screenshots.map((s) => (
                        <a key={s.id} href={s.thumb} target="_blank" rel="noreferrer" className="group relative">
                          <img src={s.thumb} alt={s.phase} className="h-16 w-28 rounded border border-terminal-border object-cover" />
                          <span className={cn('absolute bottom-0 left-0 rounded-tr bg-black/60 px-1 text-[8px] uppercase text-white')}>
                            {s.phase}
                          </span>
                        </a>
                      ))}
