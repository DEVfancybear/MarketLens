...
export function PositionsTable() {
  ...
  const live = positions.filter((p) => p.status === 'open' || p.status === 'pending');
    ...
    <div className="min-w-0 flex-1 overflow-auto">
          ...
          {live.map((p) => {
            ...
            return (
              <tr key={p.id} className="border-t border-terminal-border [&>td]:px-2 [&>td]:py-1.5">
                ...
                <td>
                  <span style={{ color: p.side === 'long' ? 'var(--bull)' : 'var(--bear)' }}>
                    {p.side.toUpperCase()}
                  </span>
                </td>
                ...
                <td className="tabular text-ink-muted">{p.stopLoss ? fmtPrice(p.stopLoss, prec) : '—'}</td>
                <td className="tabular text-ink-muted">{p.takeProfit ? fmtPrice(p.takeProfit, prec) : '—'}</td>
                <td className="tabular text-right" style={{ color: pnl >= 0 ? 'var(--bull)' : 'var(--bear)' }}>
                  {p.status === 'open' ? fmtMoney(pnl) : '—'}
                </td>
                <td>
                  <span className={cn('rounded px-1.5 py-0.5 text-[9px] uppercase',
                    p.status === 'open' ? 'bg-bull/15 text-bull' : 'bg-choch/15 text-choch')}>
                    {p.status}
                  </span>
                </td>
                <td className="text-right">
                  <div className="flex items-center justify-end gap-1">
                    {p.status === 'open' && (
                      <>
                        <button onClick={() => closePosition(p.id, 0.5)} className="rounded bg-terminal-hover px-1.5 py-0.5 text-[9px] text-ink hover:bg-brand hover:text-white" title="Close 50%">½</button>
                        <button onClick={() => closePosition(p.id, 1)} className="rounded bg-terminal-hover px-1.5 py-0.5 text-[9px] text-ink hover:bg-bear hover:text-white" title="Close 100%">×</button>
                      </>
                    )}
                    {p.status === 'pending' && (
                      <button onClick={() => cancelPending(p.id)} title="Cancel">
      ...
      {live.some((p) => p.status === 'pending') && (
        <div className="px-2 py-1 text-[9px] text-ink-faint">
          Pending orders fill as replay reveals price — {fmtDateTime(useTradeStore.getState().time)}
        </div>
      )}
    </div>
  );
