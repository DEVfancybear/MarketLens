...
export function TradePanel() {
  ...
  const startingEquity = useTradeStore((s) => s.startingEquity);
    ...
    <div className="flex h-full">
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <div className="flex h-9 shrink-0 items-center gap-4 border-b border-terminal-border px-3 text-2xs">
          ...
          <Stat label="Open P/L" value={fmtMoney(openPnl)} accent={openPnl >= 0 ? 'var(--bull)' : 'var(--bear)'} />
          <Stat label="Return" value={`${netReturn >= 0 ? '+' : ''}${netReturn.toFixed(2)}%`} accent={netReturn >= 0 ? 'var(--bull)' : 'var(--bear)'} />
          <button
            onClick={reset}
...
function Stat({ label, value, accent }: { label: string; value: string; accent?: string }) {
  return (
    <div className="flex items-center gap-1.5">
      ...
      <span className="tabular font-semibold text-ink" style={{ color: accent }}>{value}</span>
    </div>
  );
}
