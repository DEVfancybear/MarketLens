  ...
  return (
    <div className="flex w-[240px] shrink-0 flex-col gap-2 border-r border-terminal-border p-3">
      <div className="flex items-center justify-between">
        ...
        <span className="tabular text-xs text-ink-muted">{fmtPrice(price, prec)}</span>
      </div>
        ...
        {ORDER_TYPES.map((t) => (
          <button
            ...
            className={cn(
              ...
              type === t ? 'bg-brand/20 text-brand' : 'text-ink-muted hover:bg-terminal-hover',
            )}
          >
            {t}
          </button>
      ...
      {type !== 'market' && (
        <Field label="Entry price" value={entry} onChange={setEntry} placeholder={fmtPrice(price, prec)} />
      )}
      ...
      <div className="grid grid-cols-2 gap-x-3 gap-y-1 rounded bg-terminal-panel-2 p-2 text-2xs">
        ...
        <Metric label="Risk" value={fmtMoney(metrics.riskAmount)} accent="var(--bear)" />
        <Metric label="Reward" value={fmtMoney(metrics.rewardAmount)} accent="var(--bull)" />
        <Metric label="R:R" value={metrics.riskReward ? `${metrics.riskReward.toFixed(2)}` : '—'} accent="var(--accent)" />
      </div>
...
function Metric({ label, value, accent }: { label: string; value: string; accent?: string }) {
  return (
    <div className="flex items-center justify-between">
      ...
      <span className="tabular font-semibold" style={{ color: accent }}>{value}</span>
    </div>
  );
}
