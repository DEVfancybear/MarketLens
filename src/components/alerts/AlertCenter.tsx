...
import { Bell, X, Trash2, RotateCcw, Volume2, Monitor, MessageSquare } from 'lucide-react';
...
export function AlertCenter() {
  ...
  const setOpen = useUIStore((s) => s.setAlertCenter);
      ...
      >
        ...
        <div className="flex h-10 shrink-0 items-center justify-between border-b border-terminal-border px-3">
          <div className="flex items-center gap-2 text-sm font-semibold text-ink">
            ...
            Alerts
            {alerts.length > 0 && (
              <span className="rounded-full bg-terminal-hover px-1.5 text-2xs text-ink-muted">{alerts.length}</span>
            )}
          </div>
          <button
            onClick={() => setOpen(false)}
        ...
        <div className="min-h-0 flex-1 space-y-4 overflow-auto p-3">
          ...
          <div className="flex flex-wrap gap-1.5">
            <Toggle on={settings.toast} onClick={() => setSettings({ toast: !settings.toast })} icon={<MessageSquare size={12} />} label="Toast" />
            <Toggle on={settings.sound} onClick={() => setSettings({ sound: !settings.sound })} icon={<Volume2 size={12} />} label="Sound" />
            <Toggle
          ...
          <div className="space-y-2 rounded-md border border-terminal-border bg-terminal-panel-2 p-3">
            <div className="text-2xs font-semibold uppercase tracking-wide text-ink-faint">New alert</div>
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <label className="flex flex-col gap-1">
                <span className="text-2xs text-ink-faint">Target price{livePrice != null ? ` · live ${fmtPrice(livePrice, prec)}` : ''}</span>
                <input
                  type="number"
                  inputMode="decimal"
                  value={price}
          ...
          <Section title={`Active (${alerts.length})`}>
            ...
            {alerts.map((a) => (
              <div key={a.id} className="flex items-center gap-2 rounded border border-terminal-border bg-terminal-panel-2 px-2.5 py-1.5">
                <span className="font-mono text-xs font-semibold text-ink">{a.symbol}</span>
                <span className="text-2xs text-ink-muted">{CONDITION_SYMBOL[a.condition]} {fmtPrice(a.price, getMarketSymbol(a.symbol)?.pricePrecision ?? 2)}</span>
                {a.recurring && <span className="rounded bg-terminal-hover px-1 text-[9px] uppercase text-ink-faint">repeat</span>}
                <button
          ...
          {/* Triggered */}
          {triggered.length > 0 && (
            <Section title={`Triggered (${triggered.length})`} onClear={clearTriggered}>
              {triggered.map((a) => (
                <div key={a.id} className="flex items-center gap-2 rounded border border-choch/30 bg-choch/5 px-2.5 py-1.5">
                  <span className="font-mono text-xs font-semibold text-ink">{a.symbol}</span>
                  <span className="text-2xs text-ink-muted">
                    {CONDITION_SYMBOL[a.condition]} {fmtPrice(a.price, getMarketSymbol(a.symbol)?.pricePrecision ?? 2)}
                    {a.triggerPrice != null && <span className="text-choch"> @ {fmtPrice(a.triggerPrice, getMarketSymbol(a.symbol)?.pricePrecision ?? 2)}</span>}
                  </span>
                  <div className="ml-auto flex items-center gap-0.5">
                    <button onClick={() => resetAlert(a.id)} className="rounded p-1 text-ink-faint hover:bg-terminal-hover hover:text-brand" aria-label="Re-arm alert">
                      <RotateCcw size={13} />
                    </button>
                    <button onClick={() => deleteAlert(a.id)} className="rounded p-1 text-ink-faint hover:bg-terminal-hover hover:text-bear" aria-label="Delete alert">
                      <Trash2 size={13} />
                    </button>
                  </div>
                </div>
          ...
          {/* History */}
          {history.length > 0 && (
            <Section title={`History (${history.length})`} onClear={clearHistory}>
              {history.map((h) => (
                <div key={h.id} className="flex items-center gap-2 px-1 py-1 text-2xs">
                  <span className="font-mono font-semibold text-ink">{h.symbol}</span>
                  <span className="text-ink-faint">{CONDITION_SYMBOL[h.condition]} {fmtPrice(h.targetPrice, getMarketSymbol(h.symbol)?.pricePrecision ?? 2)}</span>
                  <span className="text-choch">@ {fmtPrice(h.triggerPrice, getMarketSymbol(h.symbol)?.pricePrecision ?? 2)}</span>
                  <span className="ml-auto text-ink-faint">{fmtDateTime(h.triggerTime)}</span>
                </div>
              ))}
            </Section>
          )}
...
}: { title: string; children: React.ReactNode; onClear?: () => void }) {
  return (
    <div>
      <div className="mb-1 flex items-center justify-between">
        <span className="text-2xs font-semibold uppercase tracking-wide text-ink-faint">{title}</span>
        {onClear && (
          <button onClick={onClear} className="text-[10px] text-ink-faint hover:text-bear">Clear</button>
        )}
      </div>
...
function Empty({ children }: { children: React.ReactNode }) {
  return <div className="rounded border border-dashed border-terminal-border px-2.5 py-2 text-2xs text-ink-faint">{children}</div>;
}
