...
const trendColor = (t: string) =>
  t === 'bullish' ? 'var(--bull)' : t === 'bearish' ? 'var(--bear)' : 'var(--text-muted)';
  ...
  if (!r.active) {
    return (
      <div className="p-3 text-2xs text-ink-faint">
        Replay is idle. Start it from the controls above or the toolbar. The engine hides every
        candle after the cursor — no look-ahead, no future-data leakage.
      </div>
    );
  }
    ...
    <div className="space-y-3 p-3">
      ...
      <div className="flex items-center gap-3">
        ...
        <input
          ...
          max={r.total - 1}
          value={r.cursor}
          onChange={(e) => { r.pause(); r.setCursor(Number(e.target.value)); }}
      ...
      <div className="grid grid-cols-3 gap-2 md:grid-cols-6">
        ...
        <Stat label="Price" value={current ? fmtPrice(current.close, prec) : '—'} />
        <Stat label="Trend" value={trend.toUpperCase()} accent={trendColor(trend)} />
        <Stat label="Session" value={session} accent="var(--choch)" />
      </div>

      {/* SMC active counts */}
      <div className="grid grid-cols-3 gap-2 md:grid-cols-6">
        <Stat label="Active FVG" value={snap.fvgs.filter((f) => f.state === 'active').length} accent="var(--fvg)" />
        <Stat label="Order Blocks" value={snap.orderBlocks.filter((o) => o.state !== 'invalidated').length} accent="var(--ob)" />
        <Stat label="Liquidity" value={snap.liquidity.filter((l) => !l.swept).length} accent="var(--liquidity)" />
        <Stat label="Structures" value={snap.structures.length} accent="var(--bos)" />
      ...
      {mtf.length > 0 && (
        <div>
          ...
          <div className="grid grid-cols-5 gap-2">
            {mtf.map((row) => (
              <div key={row.timeframe} className="rounded border border-terminal-border bg-terminal-panel-2 px-2 py-1.5">
                <div className="flex items-center justify-between">
                  <span className="text-2xs font-semibold text-ink">{row.timeframe}</span>
                  <span
