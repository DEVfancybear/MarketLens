...
| Managed MT5 production evidence | Implementation and local synthetic/disposable gates are complete, but production activation evidence is environment-specific. | Complete the worker/Windows-identity/credential-probe/proxy/hash/R15-9 Demo gates before Live/funded onboarding. |
...
| External services | PostgreSQL, Cloudflare/public health, Firebase, brokers, terminals, and EAs cannot be fully validated by repository-only tests; Windows credential access depends on the actual service identity. | Record environment, Windows identity alias, and exact commit for every production smoke/reconciliation result. |
