...
|---|---|---|---|---|
| G1 | ✅ | **Notification flags double-gated & stale** — _partially resolved (Phase 2.1)._ The per-alert `sound`/`browser` flags are now editable in the new **Alert Edit dialog**, so a stale flag can be corrected. The global+per-alert double-gate remains by design (global is a master switch). | Remaining (optional): drop the double-gate if confusing. |
...
| # | 🟥 | Gap | Recommendation |
|---|---|---|---|
| G5 | ✅ | **No edit UI** — _resolved (Phase 2.1)._ `AlertEditDialog` (opened from the alert right-click "Edit Alert") edits condition / price / message / recurring / enabled / sound / browser via `updateAlert`. Alerts are also draggable on the chart to change price. | — |
...
- ✅ **Edit an existing alert** and **drag the alert line on the chart** to change its price —
  _done in Phase 2.1_ (edit dialog + draggable lines + select/delete/right-click/long-press).
