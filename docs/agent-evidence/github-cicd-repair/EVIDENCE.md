...
- Source state: base commit `062a610387e7f9ed5c933d14e81ca51ee86240a6`; implementation fingerprint `frontend/package.json` SHA-256 `373E3D315CFE00DFD771567F58640A6F7E310A2D9544FBEE81B3F8F37E448A52`; gauntlet fingerprint `tools/verify-github-cicd-repair.ps1` SHA-256 `7003213B009B4ABEBF5336BD5046770EABAE90DA33021E0C61EE8EB91154A4AE`.
...
- Independent verification: pending the fresh GitHub-hosted CI run for the repair commit; completion remains blocked until it is green.

### Spec → Test mapping

| Scenario | Test | Status |
|---|---|---|
| Trade tests run as individual test files on Ubuntu Node 22 | local gauntlet `npm run test:trade`; fresh GitHub `replay-client-boundary` job | local pass; GitHub pending |
| Trade tests retain the Ky CommonJS preload bridge | `npm run test:trade` plus no-preload negative control | pass |
| Complete CI pipeline is green | fresh GitHub CI run for the pushed repair commit | unverified pending push |
...
- Verifier: GitHub-hosted Ubuntu runner with Node 22, pending push.
- Rounds: 0 completed for the repair commit at the time this evidence file was written.
...
- Fixed after the last verified state, therefore unverified: none locally; GitHub-hosted acceptance is pending.
...
- No commit, push, deployment, or production-host mutation had occurred when this report was written.
