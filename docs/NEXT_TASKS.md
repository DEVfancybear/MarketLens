...
Step 1 (codebase analysis) is **complete** — see `CURRENT_STATE.md`. Remaining steps, mapped
to concrete files/integration seams in this repo:

| Step | Task | Create / Touch | Notes |
|---|---|---|---|
| 2 | Market data types | `src/types/marketData.ts` | `MarketQuote, MarketCandle, MarketSymbol, ConnectionStatus, Timeframe`. Reuse existing `Timeframe`/`Candle` from `types/market.ts` or supersede them — pick one to avoid drift. |
| 3 | Market data store | `src/stores/marketDataStore.ts` | Single source of truth: `quotes, candles, selectedSymbol, selectedTimeframe, connectionStatus, subscriptions, lastUpdate` + actions. **Reconcile with existing `chartStore`** (move symbol/timeframe/candles here; keep `chartStore` for drawings/indicators/tool). |
